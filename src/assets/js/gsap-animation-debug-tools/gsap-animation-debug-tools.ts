export function gsapAnimationDebugTools(timelineName: any, seekBackwardDuration: number, seekForwardDuration: number) {
    ...
    animationTools.appendCode(document.body,
        `<style>
            ...
            ._animation-tools-info {
                background-color:rgba(255,255,255,.5);
                border-radius:0.25em;
                display:inline-block;
                padding:0 0.5em;
            }
            ._animation-tools-playback-controls {
                opacity:0.5
            }
            ._animation-tools-playback-controls:hover {
                opacity:1
            }
            ._animation-tools-playback-controls button {
                font-size:75%
            }
        </style>
        <div class="_animation-tools">
            <div class="_animation-tools-info">
                <span id="_animation-tools-current-time"></span>
                 / <span id="_animation-tools-total-time"></span>
            </div>
    ...
    // Button presses:
    document.querySelector('#_animation-tools-play-pause')
        .addEventListener(isTouch ? 'touchstart' : 'click', animationTools.playPause);

    document.querySelector('#_animation-tools-seek-backward')
        .addEventListener(isTouch ? 'touchstart' : 'click', animationTools.seekBackward);

    document.querySelector('#_animation-tools-seek-forward')
        .addEventListener(isTouch ? 'touchstart' : 'click', animationTools.seekForward);

    document.querySelector('#_animation-tools-restart')
        .addEventListener(isTouch ? 'touchstart' : 'click', animationTools.restart);
